...
export default function PrimairePage() {
  ...
  useEffect(() => {
    async function fetchClasses() {
      ...
      const primaireClasses = data.filter(cls => cls.niveauScolaire === 'Primaire');
      setClasses(primaireClasses);
    }
    fetchClasses();
  }, []);
  ...
  const handleAddClass = async (e) => {
    ...
    const response = await fetch('/api/classes/postclasses', {
      ...
      body: JSON.stringify({ ...newClass, niveauScolaire: 'Primaire' }),
    });
  ...
  return (
    <Layout style={{ minHeight: "100vh" }}>
      ...
      <Layout style={{ marginLeft: 156 }}>
        ...
        <Content style={{ padding: '24px', minHeight: 'calc(100vh - 64px)' }}>
          <div className="flex-1 p-6 overflow-auto">
            <div className="container mx-auto">
              <h1 className="text-3xl font-bold mb-4 text-gray-900">Gestion des classes de Primaire</h1>
              <button
                onClick={() => {
                  setSelectedClass(null);
                  setShowPopup(true);
        ...
        <tr>
          <td colSpan="4" className="px-6 py-4 text-center text-gray-500 text-sm">Aucune classe ajoutée</td>
        </tr>
      )}
                  </tbody>
                </table>
