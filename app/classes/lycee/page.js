...
export default function LyceePage() {
  ...
  useEffect(() => {
    async function fetchClasses() {
      ...
      const lyceeClasses = data.filter(cls => cls.niveauScolaire === 'Lycee');
      setClasses(lyceeClasses);
    }
    fetchClasses();
  }, []);
  ...
  const handleAddClass = async (e) => {
    ...
    const response = await fetch('/api/classes/postclasses', {
      ...
      body: JSON.stringify({ ...newClass, niveauScolaire: 'Lycee' }),
    });
        ...
        <Content style={{ padding: '24px', minHeight: 'calc(100vh - 64px)' }}>
          <div className="flex-1 p-6 overflow-auto">
            <div className="container mx-auto">
              <h1 className="text-3xl font-bold mb-4 text-gray-900">Gestion des classes de Lycée</h1>
              <button
                onClick={() => {
                  setSelectedClass(null);
                  setShowPopup(true);
