...
export default function CollegePage() {
  ...
  useEffect(() => {
    async function fetchClasses() {
      ...
      const collegeClasses = data.filter(cls => cls.niveauScolaire === 'College');
      setClasses(collegeClasses);
    }
    fetchClasses();
  }, []);
  ...
  const handleAddClass = async (e) => {
    ...
    const response = await fetch('/api/classes/postclasses', {
      ...
      body: JSON.stringify({ ...newClass, niveauScolaire: 'College' }),
    });
        ...
        <Content style={{ padding: '24px', minHeight: 'calc(100vh - 64px)' }}>
          <div className="flex-1 p-6 overflow-auto">
            <div className="container mx-auto">
              <h1 className="text-3xl font-bold mb-4 text-gray-900">Gestion des classes du Collège</h1>
              <button
                onClick={() => {
                  setSelectedClass(null);
                  setShowPopup(true);
