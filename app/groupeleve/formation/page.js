...
export default function ClassesPage() {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        const response = await fetch('/api/classes/getclasses?niveauScolaire=Formation');
        if (!response.ok) {
        ...
        <Content style={{ padding: '24px', minHeight: 'calc(100vh - 64px)' }}>
          <div className="flex-1 p-6 overflow-auto">
            <div className="container mx-auto">
              <h1 className="text-3xl font-bold mb-4 text-gray-900">Liste des groupes Formation</h1>
